...
class Eval_thread():
    ...
    def run(self):
        ...
        self.LOG('#[{:6}] [{:10}]# [MAE:{:.4f}], [max-Fmeasure:{:.4f}], [mean-Fmeasure:{:.4f}], [adp-Fmeasure:{:.4f}],'
                 '[max-Emeasure:{:.4f}], [mean-Emeasure:{:.4f}], [adp-Emeasure:{:.4f}], [S-measure:{:.4f}], '
                 '[Fbw-measure:{:.4f} ].\n'.format(self.method, self.dataset, mae, max_f, mean_f, adp_f, max_e, mean_e,
                                                   adp_e, s_alpha05, fbw))

        return '[cost:{:.4f}s] [{:6}] [{:6}], MAE:{:.4f}, max-Fmeasure:{:.4f}, mean-Fmeasure:{:.4f}, ' \
               'adp-Fmeasure:{:.4f}, max-Emeasure:{:.4f}, mean-Emeasure:{:.4f}, adp-Emeasure:{:.4f}, S-measure:{:.4f},'\
               'Fbw-measure:{:.4f}\n'.format(time.time()-start_time, self.dataset, self.method, mae, max_f, mean_f,
                                             adp_f, max_e, mean_e, adp_e, s_alpha05, fbw)
